...
export class UnitListComponent implements OnInit {
  ...
  constructor(private router: Router, private _cd: ChangeDetectorRef, private unitService: UnitService) {
    //this.capabilities = JSON.parse(localStorage.getItem('capabilities'));
  }

  ngOnInit() {
    this.getAllUnitsAgGrid(10);
    this.alertMessage = this.unitService.alertMessage;
    this.unitService.status.subscribe(data => {
      this.operationStatus = true;
      if (data == "added") {
        this.operationStatusText = "New Unit Created";
        this.operationStatus = true;
      }
      else if (data == "updated") {
        this.operationStatusText = "Unit Updated";
        this.operationStatus = true;
      }
      else {
  ...
  ngOnDestroy() {
    this.unitService.changeOperation(null);
    this.unitService.alertMessage = '';
  }
  /*   onCellKeyDown(event) {
  ...
  getSipAllQuestion() {
    ...
    this.unitService.commonGETCall(AppSettingsModule.unit + '?size=' + this.datasizeCount * 100 + '&pageNo=' + this.pagenumber + '&orderBy=' + this.orderbyfield + '&direction=' + this.reversesort).subscribe((data: any) => {
  ...
  getAllUnitsAgGrid(pageSize) {
    this.isUnitLoaded = false;
    this.unitService.commonGETCall(AppSettingsModule.unit + '?pageNo=1&size=15000&orderBy=id&direction=asc').subscribe((data: any) => {
