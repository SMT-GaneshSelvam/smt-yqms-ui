...
export class DisciplineListComponent implements OnInit {
      ...
      constructor(private router: Router, private _cd: ChangeDetectorRef, private disciplineService: DisciplineService) {
        //this.capabilities = JSON.parse(localStorage.getItem('capabilities'));
      }
    
      ngOnInit() {
        this.getAllTypesAgGrid(10);
        this.alertMessage = this.disciplineService.alertMessage;
        this.disciplineService.status.subscribe(data => {
          this.operationStatus = true;
          if (data == "added") {
            this.operationStatusText = "New Discipline Created";
            this.operationStatus = true;
          }
          else if (data == "updated") {
            this.operationStatusText = "Discipline Updated";
            this.operationStatus = true;
          }
          else {
      ...
      ngOnDestroy() {
        this.disciplineService.changeOperation(null);
        this.disciplineService.alertMessage = '';
      }
      /*   onCellKeyDown(event) {
      ...
      getSipAllQuestion() {
        ...
        this.disciplineService.commonGETCall(AppSettingsModule.discipline + '?size=' + this.datasizeCount * 100 + '&pageNo=' + this.pagenumber + '&orderBy=' + this.orderbyfield + '&direction=' + this.reversesort).subscribe((data: any) => {
      ...
      getAllTypesAgGrid(pageSize) {
        this.isUnitLoaded = false;
        this.disciplineService.commonGETCall(AppSettingsModule.discipline + '?pageNo=1&size=15000&orderBy=id&direction=asc').subscribe((data: any) => {
