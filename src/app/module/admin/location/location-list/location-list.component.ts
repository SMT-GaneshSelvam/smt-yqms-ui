...
export class LocationListComponent implements OnInit {
  ...
  constructor(private router: Router, private _cd: ChangeDetectorRef, private locationService: LocationService) {
    //this.capabilities = JSON.parse(localStorage.getItem('capabilities'));
  }

  ngOnInit() {
    this.getAllUnitsAgGrid(10);
    this.alertMessage = this.locationService.alertMessage;
    this.locationService.status.subscribe(data => {
      this.operationStatus = true;
      if (data == "added") {
        this.operationStatusText = "New Location Created";
        this.operationStatus = true;
      }
      else if (data == "updated") {
        this.operationStatusText = "Location Updated";
        this.operationStatus = true;
      }
      else {
  ...
  ngOnDestroy() {
    this.locationService.changeOperation(null);
    this.locationService.alertMessage = '';
  }
  /*   onCellKeyDown(event) {
  ...
  getSipAllQuestion() {
    ...
    this.locationService.commonGETCall(AppSettingsModule.unit + '?size=' + this.datasizeCount * 100 + '&pageNo=' + this.pagenumber + '&orderBy=' + this.orderbyfield + '&direction=' + this.reversesort).subscribe((data: any) => {
  ...
  getAllUnitsAgGrid(pageSize) {
    this.isUnitLoaded = false;
    this.locationService.commonGETCall(AppSettingsModule.location + '?pageNo=1&size=15000&orderBy=id&direction=asc').subscribe((data: any) => {
      if (data && data.length > 0) {
        ...
        this.gridOptions.columnDefs = [
...

          {
            ...
            suppressResize: true,
            suppressMovable: true,
            tooltipField: "addedAt",
            headerTooltip: "Added At",
            sortingOrder: ["asc", "desc"],
            icons: {
              sortAscending: '<span class="glyphicon glyphicon-sort-by-attributes"></span>',
              sortDescending: '<span class="glyphicon glyphicon-sort-by-attributes-alt"></span>'
          ...
          },
          {
            ...
            suppressResize: true,
            suppressMovable: true,
            tooltipField: "adddedBy",
            headerTooltip: "Addded By",
            sortingOrder: ["asc", "desc"],
            icons: {
              sortAscending: '<span class="glyphicon glyphicon-sort-by-attributes"></span>',
              sortDescending: '<span class="glyphicon glyphicon-sort-by-attributes-alt"></span>'
          ...
          },
          {
            ...
            unSortIcon: true,
            suppressMovable: true,
            tooltipField: "imported",
            headerTooltip: "Imported",
            comparator: function (a, b) {
              if (typeof a === 'string') {
                return a.localeCompare(b);
              } else {
  ...
  public changePagination = (pageSize) => {
    if (!this.loadAll) {
      ...
      this.getAllUnitsAgGrid(pageSize);
    }
  }
