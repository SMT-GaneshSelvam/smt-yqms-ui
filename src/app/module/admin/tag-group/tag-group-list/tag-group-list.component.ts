...
export class TagGroupListComponent implements OnInit {
    ...
    constructor(private router: Router, private _cd: ChangeDetectorRef, private tagGroupService: TagGroupService) {
      //this.capabilities = JSON.parse(localStorage.getItem('capabilities'));
    }
  
    ngOnInit() {
      this.getAllTagGroupsAgGrid(10);
      this.alertMessage = this.tagGroupService.alertMessage;
      this.tagGroupService.status.subscribe(data => {
        this.operationStatus = true;
        if (data == "added") {
          this.operationStatusText = "New TagGroup Created";
          this.operationStatus = true;
        }
        else if (data == "updated") {
          this.operationStatusText = "TagGroup Updated";
          this.operationStatus = true;
        }
        else {
    ...
    ngOnDestroy() {
      this.tagGroupService.changeOperation(null);
      this.tagGroupService.alertMessage = '';
    }
    /*   onCellKeyDown(event) {
    ...
    getSipAllQuestion() {
      ...
      this.tagGroupService.commonGETCall(AppSettingsModule.unit + '?size=' + this.datasizeCount * 100 + '&pageNo=' + this.pagenumber + '&orderBy=' + this.orderbyfield + '&direction=' + this.reversesort).subscribe((data: any) => {
        this.noOfPages = data.content.totalPages;
        this.rowData = this.rowData.concat(data.content);
        this.rowData = _.uniqBy(this.rowData, "id");
  
  
        this.isTagGroupLoaded = true;
    ...
    getAllTagGroupsAgGrid(pageSize) {
      this.isTagGroupLoaded = false;
      this.tagGroupService.commonGETCall(AppSettingsModule.tagGroup + '?pageNo=1&size=15000&orderBy=id&direction=asc').subscribe((data: any) => {
    ...
    public changePagination = (pageSize) => {
      if (!this.loadAll) {
        ...
        this.getAllTagGroupsAgGrid(pageSize);
      }
    }
